...
DB_PATH = PROJECT_ROOT / 'db' / 'mock_db.db'
...
def query_db(conn:sqlite3.Connection, sql:str, params:tuple = ()): # 新 *
    """通用查询函数"""
    cursor = conn.cursor()
    cursor.execute(sql, params)
    columns = [col[0] for col in cursor.description]  # 获取元数据，col[0] 包含类的列名
...
def close_db(conn:sqlite3.Connection): # 1个用法 新 *
    """安全关闭数据库"""
    if conn:
        conn.close()
        print('数据库连接已安全关闭。')
